...
const ScrapPlace = (props: Props): ReactElement => {
  ...
  const placeList: JSX.Element[] = props.placeArr.map((name, index) => <div key={index} className='place-list'>
    <div className='place-name' onClick={(e) => detailPlace(e)}>{name.placename}</div>
    <div className='place-address' onClick={(e) => detailPlace(e)}>{name.address}</div>
    <div className='x-box' onClick={(e) => props.deletePlace(e)}>X</div>
  </div>)
  ...
  return <div className={className}>
    {isPlaceClicked ? <div>
      ...
      <footer>
        <button onClick={detailPlace} className='go-list'>목록으로</button>
      </footer>
